...
read -r -p "${TAB3}Would you like to install an older version (v2.12.3)? <y/N> " prompt
if [[ ${prompt,,} =~ ^(y|yes)$ ]]; then
  msg_info "Downloading Nginx Proxy Manager v2.12.3"
  curl -fsSL "https://codeload.github.com/NginxProxyManager/nginx-proxy-manager/tar.gz/v2.12.3" | tar -xz
  cd ./nginx-proxy-manager-2.12.3
  msg_ok "Downloaded Nginx Proxy Manager v2.12.3"
else
...
if [[ ${prompt,,} =~ ^(y|yes)$ ]]; then
  sed -i "s|\"version\": \"0.0.0\"|\"version\": \"2.12.3\"|" backend/package.json
  sed -i "s|\"version\": \"0.0.0\"|\"version\": \"2.12.3\"|" frontend/package.json
else
